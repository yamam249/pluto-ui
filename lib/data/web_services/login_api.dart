...
class LoginApi {
  // Use the central Dio instance
  final Dio _dio = DioFactory.getDio();

  Future<LoginResponseModel> loginUser(LoginRequestModel user) async {
    try {
      // No 'Authorization' header needed here as we are logging in
      ...
      if (statusCode == 401) {
        throw ApiException(
          responseData?['message'] ?? 'رقم الهاتف أو كلمة المرور غير صحيحة.',
        );
      } else if (statusCode == 422) {
        _handleValidationException(e);
      }

      throw ApiException(responseData?['message'] ?? "فشل الاتصال بالسيرفر.");
    }
  }
  ...
  Future<void> logout(String token) async {
    try {
      ...
      print('⚠️ API Logout Error: $e');
      // We usually ignore logout errors and just clear local storage anyway
    }
  }
